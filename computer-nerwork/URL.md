...
3. **DNS 查询顺序**
  ...
  * 由本地 DNS 服务器去一步步访问：
    ...
    * 向域名服务器发送请求
4. **进程、线程：**
> 进程：是 CPU 资源分配的最小单位；程序的一次执行实例，拥有独立的内存空间和系统资源；   
线程：是 CPU 调度的最小单位；进程内的一个执行单元，共享进程的资源，一个进程中至少包含一个线程。可以是单线程，也可以是多线程；   
**单线程**代表处理的任务是按照一定的顺序的，前面的执行完才到后面；    
**多线程**即在一个程序中可以同时运行多个不同的 线程 来执行不同的任务， 也就是说允许单个程序创建多个并行执行的 线程 来完成各自的任务   

5. **浏览器中包含的进程**
>   * Browser 进程：浏览器的主进程；主要负责界面显示、用户交互、子进程管理等。浏览器进程内部会启动多个线程处理不同的任务（比如管理地址栏的 UI 线程来处理 url）
...
2. 首先输入 url 敲下回车之后，浏览器主进程的 UI 线程会去解析处理 url，一个 url 通常包括协议、域名、端口号、路径、请求参数等，判断是正确的 URL 之后，接着就交给网络进程发送请求。
![](./image/image1.png)
...
##### 2、页面渲染阶段
1. 客户端浏览器的网络进程拿到 HTML 文档之后，会产生一个渲染任务，将其交给渲染进程的渲染主线程消息队列；通过事件循环机制，渲染主线程取出渲染任务，开始了渲染流程，首先是 HTML 的解析阶段（parse HTML）
    ...
    * 当解析过程中遇到了普通的 script 标签，也就是 js 代码，就会停止解析，去等待网络线程下载完 js 代码，以及等待 js 引擎线程去执行 js 代码，直到 js 执行完成；
      > 渲染主线程必须等待 js 执行完成的原因是，js 代码有可能去更改 DOM 结构
      > 但是如果执行时遇到了异步代码，那么异步代码不会阻塞，而是会继续往下执行
    * 如果 script 标签加上了 async 或者 defer 属性，那么 script 的加载不会阻塞 HTML 的渲染，都会继续往下执行；等到加载完成后，async 会立即执行，有可能是在解析 HTML 的过程中；defer 则会等到 HTML 解析完成，再去执行，并且会按照 script 在 HTML 的位置顺序去执行
      > script 标签加上 defer 是会等到 HTML 解析完成之后再执行，但是解析完不代表就能渲染，还是需要等待 js 同步代码执行完之后才进行渲染。所以设置 defer 是不阻塞 HTML 解析，而对于渲染，如果 script 中的 js 执行过久，还是会阻塞 HTML 的渲染。
